// eslint-disable-next-line @typescript-eslint/no-explicit-any -- window.__RTF_MANUELL_FE_ENV__ is injected at runtime via runtime-config.js and has no type definition
function readRuntimeGlobal(): any {
  return (window as any).__RTF_MANUELL_FE_ENV__ ?? {};
}
...
// index.html, so that <script> tag never runs and window.__RTF_MANUELL_FE_ENV__
